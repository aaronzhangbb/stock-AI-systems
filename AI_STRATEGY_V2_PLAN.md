...
**买入条件**: 最终AI评分 ≥ AUTO_SCORE_THRESHOLD (默认75，可通过 .env 配置)
...
❶ 止损 (urgency=2, 立即卖出)
   └── 亏损 ≥ STOP_LOSS_PCT (默认8%)
   └── 时间衰减止损: 持仓越久，容忍亏损越小（含Phase3保护：不因时间强卖盈利仓）

❷ 止盈 (urgency=2, 立即卖出)
   └── 盈利 ≥ TAKE_PROFIT_PCT (默认20%)

❸ 追踪止损 (urgency=2, 立即卖出)
   └── 从最高价回落 ≥ TRAILING_STOP_PCT (默认5%)，且在盈利状态

❹ 策略卖出信号 + AI评分确认 (urgency=0~1, 需AI确认)
   ├── 有AI评分时 (模拟交易持仓):
   │   ├── AI评分衰减确认: 买入评分 - 当前评分 ≥ AI_SELL_SCORE_DROP (默认15)
   │   │   → urgency=1, 建议卖出 (模拟交易自动执行)
   │   ├── 多信号共振: ≥2 个技术面卖出信号同时出现
   │   │   → urgency=1, 建议卖出 (模拟交易自动执行)
   │   └── 单个信号 + AI评分未大幅下降
   │       → urgency=0, [AI未确认,仅提醒] (不卖出，等待更多确认)
   └── 无AI评分时 (手动持仓):
       └── 技术面信号直接触发 → urgency=1, 建议卖出 (仅提醒，不自动执行)
```

**AI评分全量存储**:
- `ai_daily_scores.json` 保存所有参与评分的股票（非仅 top50）
- 确保持仓股在卖出检查时始终能匹配到当前 AI 评分
- `all_scores` 字段存储全量，`top50` 字段存储前50名（兼容旧版）
...
| 页面 | 卖出信号展现 | 是否自动执行 |
|------|------------|------------|
| 模拟交易 > AI自动交易 | AI评分确认后的卖出信号 | 是 (自动卖出) |
| 我的持仓 > 账户总览 | 技术面信号直接展示提醒 | 否 (仅黄色卡片提醒) |

**卖后行情追踪 (策略进化)**:
- 自动追踪已卖出股票的5/10/20天后续行情
- 判定标签: 卖对了 / 卖早了 / 卖晚了 / 待观察
- 按卖出原因分组统计，反馈优化卖出策略
...
**买入条件**: 超级评分 ≥ AUTO_SCORE_THRESHOLD (默认75)
**卖出条件**: 五层退出机制（止损→止盈→追踪止损→策略信号+AI评分衰减确认→时间衰减），详见第七章卖出策略
